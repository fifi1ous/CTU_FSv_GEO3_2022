...
M=[C,B5001];
%% měřítko
dm=sqrt((Y(1)-Y(2))^2+(X(1)-X(2))^2);
ds=sqrt((SS(2,2)-SS(3,2))^2+(SS(2,3)-SS(3,3))^2);
roz=ds-dm;
q=ds/dm;
%% pootočení
w1=atan2(SS(3,2)-SS(2,2),SS(3,3)-SS(2,3));
w2=atan2(M(3,2)-M(2,2),M(3,3)-M(2,3));
if w1<0
    w1=w1+2*pi;
end
if w2<0
...
w=w1-w2;
%% posunutí
tx=SS(2,3)-q*(M(2,3)*cos(w)-M(2,2)*sin(w));
ty=SS(2,2)-q*(M(2,3)*sin(w)+M(2,2)*cos(w));
for n=1:size(M,1)
    VYS(n,1)=M(n,1);
